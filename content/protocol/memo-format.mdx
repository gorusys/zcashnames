...
This page is the normative grammar for every ZNS memo. The reference implementation is [`src/memo.rs`](https://github.com/zcashme/ZNS/blob/master/src/memo.rs) in the indexer repo.
...
- `<name>` - 1–62 characters of lowercase ASCII letters and digits only. No hyphens, no underscores, no unicode. See [Name Validation](/docs/protocol/name-validation).
- `<ua>`, `<new_ua>`, `<buyer_ua>` - the exact bech32m unified-address string, byte-for-byte.
- `<nonce>`, `<price>`, `<p1>…<pN>`, `<count>` - decimal ASCII, no leading zeros, no sign, no padding.
- `<sig>` - 64-byte Ed25519 signature encoded as **standard base64 with padding** (88 characters). Not base64url.
- Pricing tiers are in multiples of 10,000 zats (the indexer multiplies on the way out).
...
- Pricing has been initialized at least once (a `SETPRICE` has been observed on chain).
...
On success: insert a row with `nonce = 0` and `last_action = "CLAIM"`.
...
Rewrite the address a name resolves to.
...
- Signature over `SETPRICE:{count}:{p1}:…:{pN}:{nonce}` verifies.
