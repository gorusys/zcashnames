...
Formatted as a zero-padded 6-digit decimal string. Reference implementation from [`lib/payment/otp.ts`](https://github.com/zcashme/zcashnames/blob/main/lib/payment/otp.ts):
...
The verifier re-parses the memo, rejects if the `userAddress` field doesn't match the address it expects to be authorizing, recomputes the OTP, and compares using a constant-time equality check. From [`verifyOtp`](https://github.com/zcashme/zcashnames/blob/main/lib/payment/otp.ts):
