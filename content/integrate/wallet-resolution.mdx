...
The minimum viable ZcashNames integration: detect a name in the recipient field, call resolve, substitute the returned address, and proceed with the existing send flow.

To support ZcashNames in a wallet, you do not need to change how transactions are built or signed. You add one lookup in front of your existing recipient parser.
...
The full procedure and threat model live at [Trust Model](/docs/protocol/trust-model). The SDK does not yet ship a `verifyRegistration` helper, so you bring your own Ed25519 library (`@noble/ed25519` works in browsers and Node).
