...
Query the activity log. Returns registration, listing, and ownership events with optional filters.
...
|-------|------|-------------|
| `name` | string | Filter events for a specific name |
| `action` | string \| string[] | Filter by action type: `CLAIM`, `BUY`, `LIST`, `DELIST`, `UPDATE` |
| `since_height` | number | Only return events at or above this block height |
...
Events are returned newest-first. The `total` field gives the full count matching the filters (before `limit`/`offset`), useful for pagination.
